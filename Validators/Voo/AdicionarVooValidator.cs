...
namespace APICiaAerea.Validators.Voo
{
    public class AdicionarVooValidator : AbstractValidator<AdicionarVooViewModel>
    {
        ...
        public AdicionarVooValidator(CiaAreaContext context)
        {
            _context = context;
            ...
            RuleFor(v => v)
                .Must(voo => voo.DataHoraPartida > DateTime.Now).WithMessage("A data/hora de partida deve ser superior a data/hora atuais.")
                .Must(voo => voo.DataHoraChegada > voo.DataHoraPartida).WithMessage("A data/hora de chegada deve ser superior á data/hora de partida.");
